import {AksharaTokenizerKannada} from './akshara_tokenizer/akshara_tokenizer.js'

function prastara(string) {
    let tokenizer = new AksharaTokenizerKannada(true);
    let tokens = tokenizer.tokenize(string);
    let mapper = (token, index, tokens) => {
        let value = '';
        let current_token = token;
        if (token['virama']) {

        } else if (index + 1 < tokens.length && (tokens[index + 1]['samyukta'] || tokens[index + 1]['virama'])) {
            value = '—';
            if (tokens[index + 1]['virama'])
                current_token['akshara'] += tokens[index + 1]['akshara'];
        } else if (token['yogawaha'])
            value = '—';
        else if (token['svara'])
            value = ['ಅ', 'ಇ', 'ಉ', 'ಋ', 'ಌ', 'ಎ', 'ಒ'].includes(token['svara']) ? 'U' : '—';
        else if (token['vyamjana'])
            value = (!token['gunita'] || ['\u0cbf', '\u0cc1', '\u0cc3', '\u0cc6', '\u0cca', '\u0ce2'].includes(token['gunita'])) ? 'U' : '—';
        return [value, current_token];
    };
    let result = tokens.filter(x => x['is_akshara']).map(mapper).filter(pair => pair[0]);
    return {
        "prastara": result.map(pair => pair[0]),
        "tokens": result.map(pair => pair[1])
    }
}

export {prastara}
